// Lightweight value / colour tweening utilities (courtesy of a friend).
//
// Each tween spins up a temporary engine system that lerps a value over
// `duration` seconds using one of the easing curves below, calling `onUpdate`
// every frame and `onComplete` once when it lands.  The system removes itself
// when finished.
//
// Enhancement over the original: tweenValue / tweenColor RETURN their system
// function so a caller can cancel an in-flight tween via engine.removeSystem(...)
// before starting a new one (used by the countdown timer, which can re-trigger
// while a previous colour tween is still running).
...
export function tweenColor(
  ...
  // Already at the target — fire once and skip spinning up a system.
