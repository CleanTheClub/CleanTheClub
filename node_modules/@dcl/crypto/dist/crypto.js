...
function recoverPublicKey(signature, hash) {
    ...
    const pubKey = secp256k1_1.secp256k1.Signature.fromCompact(sigOnly)
...
function sign(privateKey, hash) {
    const sigObj = secp256k1_1.secp256k1.sign(hash, privateKey);
...
function computeAddress(key) {
    ...
    if (key.length === 65 && key[0] === 0x04) {
        ...
        throw new Error(`Invalid public key length: ${key.length}. Expected 64 bytes, or 65 bytes with a leading 0x04 uncompressed prefix.`);
    }
...
function createUnsafeIdentity() {
    ...
    // remove leading 0x04
    const publicKey = secp256k1_1.secp256k1.getPublicKey(privateKey, false).slice(1);
